...
# ============================================================
# RGPD — profil, export, suppression
# ============================================================
...
async def get_me(user_id: str = Depends(get_current_user_id)):
    ...
    try:
        res = c.table("users").select("*").eq("id", user_id).execute()
        return success_response(res.data[0] if res.data else {"id": user_id}, "OK")
    except Exception as e:
        return error_response(str(e), 500)


@router.put("/me")
async def update_me(payload: Dict[str, Any], user_id: str = Depends(get_current_user_id)):
    """Met à jour le profil de l'utilisateur courant."""
    c = get_supabase_admin()
    if not c:
        return error_response("Base non disponible", 503)
    try:
        allowed_fields = {"display_name", "avatar_url", "phone_number", "preferences"}
        update_data = {k: v for k, v in payload.items() if k in allowed_fields}
        if not update_data:
            return error_response("Aucun champ valide fourni", 400)
        c.table("users").update(update_data).eq("id", user_id).execute()
        return success_response({"updated": True}, "Profil mis à jour")
    except Exception as e:
        return error_response(str(e), 500)


@router.delete("/me/data")
async def delete_my_data(user_id: str = Depends(get_current_user_id)):
    """
    RGPD — Supprime toutes les données de l'utilisateur en cascade.
    """
    c = get_supabase_admin()
    if not c:
        return error_response("Base non disponible", 503)
    try:
        tables_to_clear = [
            "messages", "chat_sessions", "memory_snippets", "generated_images",
            "generated_videos", "video_analyses", "inbound_calls", "outbound_calls",
            "sms_log", "social_posts", "social_accounts", "file_index", "avatars",
            "avatar_conversations", "screen_sessions", "video_surveillance_sessions",
            "proactive_calls", "proactivity_settings", "call_settings", "moderation_logs",
        ]
        ...
        return success_response({"deleted": True, "user_id": user_id}, "Toutes les données ont été supprimées")
    except Exception as e:
        return error_response(str(e), 500)


@router.get("/me/export")
async def export_my_data(user_id: str = Depends(get_current_user_id)):
    """
    ...
    if not c:
        return error_response("Base non disponible", 503)
    try:
        export_data: Dict[str, Any] = {"user_id": user_id}

        tables = {
            "profile": ("users", "id"),
            "conversations": ("chat_sessions", "user_id"),
            "messages": ("messages", None),  # via sessions
            "images": ("generated_images", "user_id"),
            "videos": ("generated_videos", "user_id"),
            "sms_log": ("sms_log", "user_id"),
            "calls": ("inbound_calls", "user_id"),
            "social_posts": ("social_posts", "user_id"),
            "files": ("file_index", "user_id"),
            "memory": ("memory_snippets", "user_id"),
        }

        for key, (table, user_col) in tables.items():
            if user_col:
                try:
                    res = c.table(table).select("*").eq(user_col, user_id).execute()
                    export_data[key] = res.data or []
                except Exception:
                    export_data[key] = []

        from fastapi.responses import Response
        return Response(
            content=json_module.dumps(export_data, ensure_ascii=False, default=str),
            media_type="application/json",
            headers={
                "Content-Disposition": f'attachment; filename="chadgpt_export_{user_id[:8]}.json"'
            },
        )
    except Exception as e:
        return error_response(str(e), 500)
